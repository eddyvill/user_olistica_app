...
Future<ConfigurationResponse> getAppConfigurations({bool isCurrentLocation = false, double? lat, double? long}) async {
  ...
  try {
    ConfigurationResponse? data = ConfigurationResponse.fromJson(await handleResponse(await buildHttpResponse('configurations', method: HttpMethod.GET)));

    if (data.appDownload != null) {
      setValue(APPSTORE_URL, data.appDownload!.appstore_url.validate());
      setValue(PLAY_STORE_URL, data.appDownload!.playstore_url.validate());
    ...
    if (data.privacyPolicy != null) {
      if (data.privacyPolicy!.value != null) {
    ...
    if (data.termConditions != null) {
      if (data.termConditions?.value != null) {
    ...
    if (data.generalSetting != null) {
      setValue(SITE_DESCRIPTION, data.generalSetting!.siteDescription.validate());
      setValue(SITE_COPYRIGHT, data.generalSetting!.siteCopyright.validate());
      setValue(FACEBOOK_URL, data.generalSetting!.facebookUrl.validate());
      setValue(INSTAGRAM_URL, data.generalSetting!.instagramUrl.validate());
      setValue(TWITTER_URL, data.generalSetting!.twitterUrl.validate());
      setValue(LINKEDIN_URL, data.generalSetting!.linkedinUrl.validate());
      setValue(YOUTUBE_URL, data.generalSetting!.youtubeUrl.validate());

      if (!data.generalSetting!.inquriyEmail.isEmptyOrNull) {
        appStore.setInquiryEmail(data.generalSetting!.inquriyEmail.validate());
      } else {
        appStore.setInquiryEmail(HELP_SUPPORT_URL);
      }

      if (!data.generalSetting!.helplineNumber.isEmptyOrNull) {
    ...
    appStore.setEnableUserWallet(data.enableUserWallet.validate());

    appStore.setUnreadCount(data.notificationUnreadCount.validate());
...
Future<List<RatingData>> customerReviews() async {
  ServiceReviewResponse res = ServiceReviewResponse.fromJson(await handleResponse(await buildHttpResponse('get-user-ratings?per_page=all', method: HttpMethod.GET)));
  return res.ratingList.validate();
}
...
Future<String> sadadLogin(Map request) async {
  var res = await handleResponse(
    await buildHttpResponse('$SADAD_API_URL/api/userbusinesses/login', method: HttpMethod.POST, request: request, extraKeys: {
      'isSadadPayment': true,
    }),
...
Future sadadCreateInvoice({required Map<String, dynamic> request, required String sadadToken}) async {
  return handleResponse(
    await buildHttpResponse('$SADAD_API_URL/api/invoices/createInvoice', method: HttpMethod.POST, request: request, extraKeys: {
      'isSadadPayment': true,
